    ...
    switch (type) {
      ...
      case "pulse": {
        ...
        meshRef.current.scale.set(
          Math.sign(sx) *
            Math.abs(sx / (1 + Math.sin((t - delta * speed) * 2) * 0.05)) *
            pulse,
          Math.sign(sy) *
            Math.abs(sy / (1 + Math.sin((t - delta * speed) * 2) * 0.05)) *
            pulse,
          Math.sign(sz) *
            Math.abs(sz / (1 + Math.sin((t - delta * speed) * 2) * 0.05)) *
            pulse,
        );
        break;
      }
      ...
      case "spinFloat":
        ...
        meshRef.current.position.y =
          initialY.current + Math.sin(t * 1.2) * 0.25;
        break;
    }
  });
...
export function SmoothControls({
  ...
  useFrame(() => {
    ...
    if (resetOnIdle && introComplete.current) {
      ...
      resetting =
        !isDragging.current &&
        (!cursorInWindow.current || idle > resetDelay * 1000);
    }
    ...
    if (resetting) {
      ...
      baseRotation.current.x +=
        (rotationX - baseRotation.current.x) * resetDamping;
      baseRotation.current.y +=
        (rotationY - baseRotation.current.y) * resetDamping;
    ...
    targetRotation.current.x =
      baseRotation.current.x + (cursorOrbit ? cursorOffset.current.x : 0);
    targetRotation.current.y =
      baseRotation.current.y + (cursorOrbit ? cursorOffset.current.y : 0);
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      targetZoom.current = Math.max(
        2,
        Math.min(20, targetZoom.current + e.deltaY * 0.01),
      );
      if (resetOnIdle) markActive();
    };
    ...
    const onTouchMove = (e: TouchEvent) => {
      if (e.touches.length === 2 && scrollZoom) {
        ...
        targetZoom.current = Math.max(
          2,
          Math.min(20, targetZoom.current + delta * 0.03),
        );
        lastPinchDist = dist;
        if (resetOnIdle) markActive();
      }
