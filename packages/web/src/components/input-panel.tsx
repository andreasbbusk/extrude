...
export type InputTab = "draw" | "text" | "code" | "file";

interface InputPanelProps {
  inputTab: InputTab;
  onInputTabChange: (tab: InputTab) => void;
  customSvg: string;
...
const tabs: Array<{ value: InputTab; icon: typeof Pencil; label: string }> = [
...
export function InputPanel({
  ...
  const handleTabClick = (value: InputTab) => {
              ...
              <div className="space-y-2">
                ...
                {!customSvg && (
                  <Button
                    ...
                    onClick={() => onCustomSvgChange(DEFAULT_STARTER_SVG)}
                  >
                    Load starter ({DEFAULT_STARTER_NAME.toLowerCase()})
                  </Button>
                )}
              </div>
            </div>
